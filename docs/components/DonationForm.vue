...
const handleDonateNow = async () => {
  ...
  try {
    localStorage.setItem('donationDetails', JSON.stringify({
      ...
      amount: customAmount.value || selectedAmount.value,
      method: 'paypal'
    }))
    
    router.go('/donate/details')
  } catch (error) {
...
const handleDirectDebit = async () => {
  ...
  processing.value = true
  try {
    // Ensure method is explicitly set to 'direct-debit'
    const donationData = {
      type: donationType.value,
      amount: customAmount.value || selectedAmount.value,
      method: 'direct-debit'  // Make sure this is set correctly
    }
    
    console.log('Storing donation data:', donationData) // Debug log
    localStorage.setItem('donationDetails', JSON.stringify(donationData))
    
    router.go('/donate/details.html')
  } catch (error) {
    console.error('Error processing donation:', error)
  } finally {
    processing.value = false
    ...
    <div class="donation-wrapper">
      ...
      <div class="donation-box">
        ...
        <div class="payment-methods">
          ...
          <img src="/images/direct-debit.svg" alt="Direct Debit" class="payment-icon">
        </div>
      </div>
    </div>
  </div>
